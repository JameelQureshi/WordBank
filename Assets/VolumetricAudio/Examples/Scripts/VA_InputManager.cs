...
namespace VolumetricAudio
{
	...
	[HelpURL(VA_Helper.HelpUrlPrefix + "VA_InputManager")]
	[AddComponentMenu("Volumetric Audio/Input Manager")]
	public class VA_InputManager : MonoBehaviour
	{
		...
		public struct Axis
		{
			...
			public float GetValue(float delta)
			{
				...
				value -= IsDown(KeyNegative) == true ? KeySensitivity * delta : 0.0f;
				value += IsDown(KeyPositive) == true ? KeySensitivity * delta : 0.0f;

				value -= IsDown(KeyNegativeAlt) == true ? KeySensitivity * delta : 0.0f;
				value += IsDown(KeyPositiveAlt) == true ? KeySensitivity * delta : 0.0f;
		...
		public struct Trigger
		{
			...
			public bool WentDown(Finger finger)
			{
				...
				if (UseKey != KeyCode.None && finger.Index == HOVER_FINGER_INDEX && VA_InputManager.WentDown(UseKey) == true)
				{
					return true;
				}
			...
			public bool IsDown(Finger finger)
			{
				...
				if (UseKey != KeyCode.None && finger.Index == HOVER_FINGER_INDEX && VA_InputManager.IsDown(UseKey) == true)
				{
					return true;
				}
			...
			public bool WentUp(Finger finger, bool useAnyFinger = false)
			{
				...
				if (UseKey != KeyCode.None && finger.Index == HOVER_FINGER_INDEX && VA_InputManager.WentUp(UseKey) == true)
				{
					return true;
				}
		...
		{
			if (Application.isPlaying == true && FindObjectOfType<VA_InputManager>() == null)
			{
				new GameObject(typeof(VA_InputManager).Name).AddComponent<VA_InputManager>();
			}
		}
