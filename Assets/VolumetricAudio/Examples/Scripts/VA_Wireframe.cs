...
namespace VolumetricAudio.Examples
{
	...
	[ExecuteInEditMode]
	[HelpURL(VA_Helper.HelpUrlPrefix + "VA_Wireframe")]
	[AddComponentMenu("Volumetric Audio/VA Wireframe")]
...
namespace VolumetricAudio.Examples
{
	...
	[CustomEditor(typeof(VA_Wireframe))]
	public class VA_Wireframe_Editor : VA_Editor<VA_Wireframe>
	{
		...
		protected override void OnInspector()
		{
			...
			BeginError(Any(t => t.Pairs.Count == 0));
				Draw("pairs");
			EndError();
			...
			{
				Each(t => { t.BuildFromMeshFilter(threshold); t.UpdateMesh(); }, true, "Build From MeshFilter");
			}
		}
	}
}
